...
rowid = np.zeros(shape=(nr_baselines, nr_timesteps), dtype=np.int)
...
img[0, (N - N0) // 2 : (N + N0) // 2, (N - N0) // 2 : (N + N0) // 2] = d[
    :, :
] / np.outer(taper_grid0, taper_grid0)
img[3, (N - N0) // 2 : (N + N0) // 2, (N - N0) // 2 : (N + N0) // 2] = d[
    :, :
] / np.outer(taper_grid0, taper_grid0)
...
flags_block = table.getcol("FLAG", startrow=start_row, nrow=nr_rows)[:, :nr_channels, :]
...
# Reshape data
antenna1_block = np.reshape(antenna1_block, newshape=(nr_timesteps, nr_baselines))
antenna2_block = np.reshape(antenna2_block, newshape=(nr_timesteps, nr_baselines))
uvw_block = np.reshape(uvw_block, newshape=(nr_timesteps, nr_baselines, 3))
vis_block = np.reshape(
    vis_block, newshape=(nr_timesteps, nr_baselines, nr_channels, nr_correlations)
)
weight_block = np.reshape(
    weight_block, newshape=(nr_timesteps, nr_baselines, nr_channels, nr_correlations)
)
rowid_block = np.reshape(rowid_block, newshape=(nr_timesteps, nr_baselines))
...
for t in range(nr_timesteps):
    for bl in range(nr_baselines):
        ...
        # TODO: what does index 51 mean here?
...
for i in range(nr_stations):
    ...
    #    aterm_derivatives = mask_aterm(np.concatenate((aterm_derivatives_ampl, aterm_derivatives_phase), axis=1), aterm_support)
    ...
    print(
        (
            ...
            residual_ref1,
            residual_ref,
            residual[0] - residual_ref1,
        )
    )

    if gradient_err > 1e-2 or hessian_err > 1e-2 or residual_err > 1e-2:
        print(("station {0} differs".format(i)))
        break
