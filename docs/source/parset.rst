...
.. glossary::
    ...
    filter_skymodel
        Filter out sky model components that lie outside of detected PyBDSF islands
        (default = ``True``). If ``True``, only clean components from WSClean whose
        centers lie inside of detected PyBDSF islands are kept in the sky model (used for
        calibration in the next cycle, if any). If ``False``, all clean components
        generated by WSClean are kept in the sky model.

        .. note::

            It is not recommneded to turn off this option unless the
            :term:`use_image_based_predict` parameter is set, as the sky model without
            filtering can be very large (resulting in runtimes becoming very long).
