...
from rapthor.scripts.restore_skymodel import main as restore_main
...
def test_integration_restore_skymodel_compressed(
...
def test_get_primary_hdu_or_compressed_with_primary_with_uncompressed_returns_primary(
    reference_image,
):
    """
...
def test_get_primary_hdu_or_compressed_with_compressed_returns_compressed(
...
def test_make_zero_image(reference_image):
    ...
    try:
        ...
        with fits.open(zero_image_path) as zero_fits, fits.open(reference_image) as ref_fits:
            zero_hdu = zero_fits[0]
            ref_hdu = ref_fits[0]
            assert pixel_scale > 0, "Pixel scale should be positive"
            assert pixel_scale == min(
                abs(ref_hdu.header["CDELT1"]) * 3600.0, abs(ref_hdu.header["CDELT2"]) * 3600.0
            ), "Pixel scale should match derived minimum scale"
