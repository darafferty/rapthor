...
def load_cwl_workflow(workflow: Union[str, Path]) -> Dict[str, Any]:
    ...
    return {"steps": steps, "inputs": cwl_data.get("inputs"), "outputs": cwl_data.get("outputs")}


def _resolve_array_base_type(
    array_object: Union[Dict[str, Any], str], suffix: str = ""
) -> Optional[str]:
...
def infer_scatter_length(
    workflow: Optional[Union[str, Path, Dict[str, Any]]],
    output_source: str,
...
def _build_mock_output(
    ...
    if output_type in ["File", "Directory"]:
        ...
        return [
            _build_mock_output(output_path, f"{base_name}_{idx}", "File")
            for idx in range(outer_size)
        ]
    elif output_type == "Directory[]":
        return [
            _build_mock_output(output_path, f"{base_name}_{idx}", "Directory")
            for idx in range(outer_size)
        ]
...
def _resolve_outer_length(
    workflow: Optional[Union[str, Path, Dict[str, Any]]],
    output_source: str,
    inputs: Optional[Dict[str, Any]],
    default_outer_size: int,
) -> int:
...
def _materialize_expected_outputs(
    output_path: Path, expected_filenames: Union[str, List[Any]]
    ...
    """
    if (
        isinstance(expected_filenames, list)
        and len(expected_filenames) > 0
        and isinstance(expected_filenames[0], list)
    ):
...
def _build_outputs_for_sources(
    output_path: Path,
    ...
    if not isinstance(output_sources, list):
        ...
        output_summary = _build_mock_output(
            output_path, base_name, output_type, inner_size=mock_n_inner, outer_size=mock_n_outer
        )
        return output_summary
    ...
    for output_source in output_sources:
        ...
        mock_output = _build_mock_output(
            output_path, base_name, output_type, inner_size=mock_n_inner, outer_size=mock_n_outer
        )
...
def build_mock_outputs(
    ...
    for output_key, output_info in outputs.items():
        ...
        if expected_outputs and output_key in expected_outputs:
            ...
            outputs_json[output_key] = _build_outputs_for_sources(
                output_path,
                output_info,
...
def mock_cwl_execution(expected_outputs: Optional[Dict[str, Any]]) -> Callable:
    ...
    def decorator(func):
        def wrapper(self, *args, **kwargs):
            ...
            self.operation.cwl_execute = lambda args, env: mocked_cwl_execution(
                self, args, env, expected_outputs=expected_outputs
            )
...
def mocked_cwl_execution(
    self: Any, args: Any, env: Any, expected_outputs: Optional[Dict[str, Any]] = None
) -> bool:
    ...
    outputs_json = build_mock_outputs(
        output_path,
        outputs,
