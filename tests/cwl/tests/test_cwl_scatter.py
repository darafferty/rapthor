...
from ..cwl_mock import build_mock_outputs, infer_scatter_length, load_cwl_workflow
...
class TestScatterAware:
    ...
    def nested_scatter_workflow(self, tmp_path):
        ...
        workflow_content = {
            ...
            "steps": [
                {
                    "id": "predict_step",
                    "run": "predict.cwl",
                    "in": {"obs": {"source": "observations"}, "sector": {"source": "sectors"}},
                    "scatter": ["obs", "sector"],
                    "scatterMethod": "dotproduct",
                    "out": ["models"],
                },
            ...
            "outputs": {
                "output_models": {
                    "type": {"type": "array", "items": {"type": "array", "items": "Directory"}},
    ...
    def test_get_scatter_length_with_inputs(self, simple_scatter_workflow):
        ...
        length = infer_scatter_length(
            simple_scatter_workflow, "process_step/processed", {"files": inputs["input_files"]}
        )
    ...
    def test_get_scatter_length_no_scatter(self, tmp_path):
        ...
        workflow_content = {
            ...
            "outputs": {"result": {"type": "File", "outputSource": "simple_step/output"}},
        }
        with open(workflow_file, "w") as f:
            yaml.dump(workflow_content, f)
    ...
    def test_generate_with_scatter_length(self, simple_scatter_workflow, tmp_path):
        ...
        inputs = {"files": [f"/path/to/file{i}.txt" for i in range(5)]}
        ...
        build_mock_outputs(output_path, outputs, workflow=simple_scatter_workflow, inputs=inputs)

        # Should create 5 files (matching scatter length) instead of default 3
        result = build_mock_outputs(
            output_path, outputs, workflow=simple_scatter_workflow, inputs=inputs
        )
    ...
    def test_generate_with_nested_scatter(self, nested_scatter_workflow, tmp_path):
        ...
        build_mock_outputs(output_path, outputs, workflow=nested_scatter_workflow, inputs=inputs)

        # Should create 4 outer directories (matching obs scatter length)
        result = build_mock_outputs(
            output_path, outputs, workflow=nested_scatter_workflow, inputs=inputs
        )
    ...
    def test_fallback_to_default_without_inputs(self, simple_scatter_workflow, tmp_path):
        ...
        build_mock_outputs(
            output_path, outputs, mock_n_outer=3, workflow=simple_scatter_workflow, inputs=None
        )
    ...
    def test_mixed_scatter_and_non_scatter(self, tmp_path):
        ...
        workflow_content = {
            ...
            "outputs": {
                ...
                "regular_result": {"type": "File", "outputSource": "regular_step/result"},
            },
        }
        with open(workflow_file, "w") as f:
            yaml.dump(workflow_content, f)
    ...
    def test_single_output_source_does_not_add_extra_nesting(self, tmp_path):
        ...
        workflow_content = {
            "cwlVersion": "v1.2",
            "class": "Workflow",
            "inputs": {"obs_filename": {"type": {"type": "array", "items": "File"}}},
            "steps": [
                {
                    "id": "image_sector",
                    "run": "image_sector.cwl",
                    "in": {"obs": {"source": "obs_filename"}},
                    "scatter": ["obs"],
                    "scatterMethod": "dotproduct",
