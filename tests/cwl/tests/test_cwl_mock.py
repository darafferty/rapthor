...
class TestGetOutputType:
    ...
    def test_nullable_array_type(self):
        ...
        output_info = {"type": ["null", {"type": "array", "items": {"type": "File"}}]}
        assert infer_output_type(output_info) == "File[]"
...
class TestGenerateMockFiles:
    ...
    def test_generate_single_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = Path(tmp_dir)
            outputs = {"output_dir": {"type": "Directory", "outputSource": "step/output_dir"}}
            result = build_mock_outputs(output_path, outputs)
    ...
    def test_generate_array_of_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            outputs = {
                "output_files": {
                    "type": {"type": "array", "items": {"type": "File"}},
    ...
    def test_generate_array_of_directories(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            outputs = {
                "output_dirs": {
                    "type": {"type": "array", "items": {"type": "Directory"}},
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            outputs = {
                "nested_files": {
                    "type": {
                        "type": "array",
                        "items": {"type": "array", "items": {"type": "File"}},
                    },
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            outputs = {
                "nested_dirs": {
                    "type": {
                        "type": "array",
                        "items": {"type": "array", "items": {"type": "Directory"}},
                    },
    ...
    def test_generate_multiple_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = Path(tmp_dir)
            outputs = {
                "file_out": {"type": "File", "outputSource": "step1/file_out"},
                "dir_out": {"type": "Directory", "outputSource": "step2/dir_out"},
                "files_out": {
                    "type": {"type": "array", "items": {"type": "File"}},
    ...
    def test_output_source_as_list(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = Path(tmp_dir)
            outputs = {"out": {"type": "File", "outputSource": ["step1/out", "step2/out"]}}
            result = build_mock_outputs(output_path, outputs)
...
class TestParseOutputsIntegration:
    ...
    def test_load_cwl_workflow(self, tmp_path):
        ...
        workflow_list = {
            ...
            "outputs": {"output1": {"type": "File", "outputSource": "step1/result"}},
            "steps": [
                {"id": "step1", "run": "tool1.cwl", "in": {"input": "input1"}, "out": ["result"]},
                {
                    "id": "step2",
                    "run": "tool2.cwl",
                    "in": {"input": "step1/result"},
                    "out": ["result"],
                },
            ],
        }
        ...
        workflow_dict = {
            ...
            "outputs": {"output1": {"type": "File", "outputSource": "step1/result"}},
            "steps": {
                "step1": {"run": "tool1.cwl", "in": {"input": "input1"}, "out": ["result"]},
                "step2": {"run": "tool2.cwl", "in": {"input": "step1/result"}, "out": ["result"]},
            },
        }
