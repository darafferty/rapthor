 ...
 * Average beam
 */
void GenericOptimized::do_compute_avg_beam(
  ...
  std::complex<double> average_beam_double[subgrid_size*subgrid_size][NR_CORRELATIONS][NR_CORRELATIONS];
...
#pragma omp parallel
{
    ...
    float sum_of_weights[nr_aterms][NR_CORRELATIONS];
    memset(sum_of_weights, 0, nr_aterms * NR_CORRELATIONS * sizeof(float));
    ...
    for (unsigned int i = tid; i < (subgrid_size * subgrid_size); i += num_threads) {
      ...
      memset(sum, 0, NR_CORRELATIONS * NR_CORRELATIONS * sizeof(std::complex<double>));
