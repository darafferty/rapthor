...
{
  assert(nr_aterms <= MAX_NR_ATERMS);
  ...
  for (unsigned int i = tid; i < (subgrid_size * subgrid_size); i += num_threads) {
    ...
    for (unsigned int n = 0; n < nr_aterms; n++) {
      ...
      if (i == tid) {
        unsigned int time_start = aterms_offsets[n];
        unsigned int time_end = aterms_offsets[n + 1];
      ...
      int station1_idx = index_aterm(subgrid_size, nr_antennas, n, antenna1, y, x, 0);
      int station2_idx = index_aterm(subgrid_size, nr_antennas, n, antenna2, y, x, 0);
