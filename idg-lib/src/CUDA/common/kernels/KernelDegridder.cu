...
inline __device__ void kernel_degridder_1_(
    const int                         grid_size,
    const int                         subgrid_size,
    const float                       image_size,
    const float                       w_step,
...
#define KERNEL_DEGRIDDER_TEMPLATE_1 \
    for (; (channel_offset + 1) <= nr_channels; channel_offset++) { \
        kernel_degridder_1_( \
            grid_size, subgrid_size, image_size, w_step, nr_channels, channel_offset, nr_stations, \
            uvw, wavenumbers, visibilities, spheroidal, aterm, metadata, subgrid); \
    }
