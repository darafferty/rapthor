          ...
          float2*        __restrict__ subgrid)
{
    ...
    for (int time = tid; time < ALIGN(nr_timesteps, nr_threads); time += nr_threads) {
        ...
        // Initialize aterm index to -1, this will trigger
        // preparation of input data for the first timestep
        int aterm_idx_global_previous = -1;

        // Iterate all timesteps currently in range
        int time_first = time - tid;
        int time_last  = min(time_first + nr_timesteps, nr_timesteps);

        for (int time_current = time_first; time_current < time_last; time_current++) {
            int aterm_idx_global_current = aterms_indices[time_offset_global + time_current];

            // Determine whether aterm has changed
            bool aterm_changed = aterm_idx_global_previous != aterm_idx_global_current;
            ...
            if (aterm_changed) {
                ...
                for (int pixel_offset = 0; pixel_offset < nr_pixels; pixel_offset += current_nr_pixels) {
                    ...
                    // Update aterm index
                    aterm_idx_global_previous = aterm_idx_global_current;

                    __syncthreads();

                    // Aterm index for the current thread
                    int aterm_idx_local = aterms_indices[time_offset_global + time];

                    // Only update visibility when aterm matches
                    if (aterm_idx_global_current == aterm_idx_local) {
