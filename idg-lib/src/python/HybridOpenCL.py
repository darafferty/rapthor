...
path, junk = os.path.split(path)
libpath = os.path.join(path, 'libidg-hybrid-opencl.so')
lib = ctypes.cdll.LoadLibrary(libpath)



class Reference(Proxy):
    """Hybrid OpenCL Reference implementation"""
    def __init__(self, nr_stations,
                       ...
                       subgrid_size):
        try:
            lib.Hybrid_OpenCL_init.argtypes = [ctypes.c_uint, \
                                                         ctypes.c_uint, \
                                                         ctypes.c_uint, \
                                                         ctypes.c_uint, \
                                                         ctypes.c_float, \
                                                         ctypes.c_uint, \
                                                         ctypes.c_uint]
            self.obj = lib.Hybrid_OpenCL_init(
    ...
    def _cwrap_grid_visibilities(self,
                                 ...
                                 aterms_offset,
                                 spheroidal):
        lib.Hybrid_OpenCL_grid(self.obj,
    ...
    def _cwrap_degrid_visibilities(self,
                                   ...
                                   aterms_offset,
                                   spheroidal):
        lib.Hybrid_OpenCL_degrid(self.obj,
                                 visibilities.ctypes.data_as(ctypes.c_void_p),
                                 uvw.ctypes.data_as(ctypes.c_void_p),
                                 wavenumbers.ctypes.data_as(ctypes.c_void_p),
                                 baselines.ctypes.data_as(ctypes.c_void_p),
    ...
    def _cwrap_transform(self, direction, grid):
        lib.Hybrid_OpenCL_transform(self.obj,
