            ...
            for (int y = start_y; y < subgrid_size; y += num_threads)
            {
                ...
                for (int x = 0; x < subgrid_size; x++) {
                    ...
                    for (int pol = 0; pol < NR_POLARIZATIONS; pol++) {
                        long src_idx = index_grid(WTILE_SIZE + subgrid_size, tile_index, pol, y_dst, x_dst);
    ...
    for (int i = 0; i<nr_tiles; i++)
    {
        ...
        for (int y = y_start; y < y_end; y++) {
            for (int x = x_start; x < x_end; x++) {
                tile_buffer[                                          (y-y0)*w_padded_tile_size + x-x0] = grid[index_grid(grid_size,0,y,x)];
                tile_buffer[  w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0] = grid[index_grid(grid_size,1,y,x)];
                tile_buffer[2*w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0] = grid[index_grid(grid_size,2,y,x)];
                tile_buffer[3*w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0] = grid[index_grid(grid_size,3,y,x)];
            }
        }
