          ...
          float*           __restrict__ average_beam_)
{
  ...
  std::complex<double> average_beam_double[subgrid_size*subgrid_size][NR_POLARIZATIONS][NR_POLARIZATIONS];
...
#pragma omp parallel
{
    ...
    float sum_of_weights[nr_aterms][NR_POLARIZATIONS];
    memset(sum_of_weights, 0, nr_aterms * NR_POLARIZATIONS * sizeof(float));
    ...
    for (unsigned int i = tid; i < (subgrid_size * subgrid_size); i += num_threads) {
      ...
      memset(sum, 0, NR_POLARIZATIONS * NR_POLARIZATIONS * sizeof(std::complex<double>));
      ...
      for (unsigned int n = 0; n < nr_aterms; n++) {
          ...
          for (unsigned int t = time_start; t < time_end; t++) {
            ...
            for (unsigned int ch = 0; ch < nr_channels; ch++) {
              for (unsigned int pol = 0; pol < NR_POLARIZATIONS; pol++) {
                sum_of_weights[n][pol] += weights[bl][t][ch][pol];
              }
            }
          } // end for time
        }
        ...
        for (int ii = 0; ii < NR_POLARIZATIONS; ii++) {
          for (int jj = 0; jj < NR_POLARIZATIONS; jj++) {
            ...
            for (int p = 0; p < NR_POLARIZATIONS; p++) {
              ...
              update_real +=
                  sum_of_weights[n][p] * (kp1_real * kp2_real - kp1_imag * kp2_imag);
              update_imag +=
                  sum_of_weights[n][p] * (kp1_real * kp2_imag + kp1_imag * kp2_real);
            }
      ...
      for (size_t ii = 0; ii < 4; ii++) {
        for (size_t jj = 0; jj < 4; jj++) {
          average_beam_double[i][ii][jj] += sum[ii][jj];
        }
      }
    } // end for pixels
} // end parallel
  ...
  for (unsigned int i = 0; i < (subgrid_size * subgrid_size); i++) {
    for (size_t ii = 0; ii < 4; ii++) {
      for (size_t jj = 0; jj < 4; jj++) {
        average_beam[i][ii][jj] = average_beam_double[i][ii][jj];
      }
    }
  }
