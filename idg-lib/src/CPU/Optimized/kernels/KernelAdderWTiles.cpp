...
extern "C" {
void kernel_adder_subgrids_to_wtiles(const long nr_subgrids,
                                     const int grid_size,
...
void kernel_adder_wtiles_to_grid(int grid_size, int subgrid_size,
                                 float image_size, float w_step, int nr_tiles,
                                 int *tile_ids,
  ...
  for (int i = 0; i < nr_tiles; i++) {
    ...
    std::fill(&tiles[tile_ids[i] * padded_tile_size * padded_tile_size *
                     NR_POLARIZATIONS],
              &tiles[(tile_ids[i] + 1) * padded_tile_size * padded_tile_size *
                     NR_POLARIZATIONS],
              idg::float2({0.0, 0.0}));
