          ...
          idg::float2*     tiles)
{
    ...
    #pragma omp parallel
    {
        ...
        for (int s = 0; s < nr_subgrids; s++) {
            ...
            int tile_top  = metadata[s].wtile_coordinate.x * WTILE_SIZE - subgrid_size/2 + grid_size/2;
            int tile_left = metadata[s].wtile_coordinate.y * WTILE_SIZE - subgrid_size/2 + grid_size/2;
    ...
    idg::float2 *grid)
{
    ...
    int max_tile_size = next_composite(padded_tile_size + int(ceil(max_abs_w*image_size*image_size)));

    std::vector<idg::float2> tile_buffer(max_tile_size * max_tile_size * NR_POLARIZATIONS);

    for (int i = 0; i<nr_tiles; i++)
    {
        idg::Coordinate &coordinate = tile_coordinates[i];
        float w = (coordinate.z + 0.5f) * w_step;
        int w_padded_tile_size = next_composite(padded_tile_size + int(ceil(std::abs(w)*image_size*image_size)));
        ...
        for(int pol = 0; pol<NR_POLARIZATIONS; pol++)
        {
            for(int x = 0; x<padded_tile_size; x++)
            {
                ...
                {
                    int y2 = y + w_padding2;
                    tile_buffer[pol*w_padded_tile_size*w_padded_tile_size + x2*w_padded_tile_size + y2] =
                        tiles[tile_ids[i]*padded_tile_size*padded_tile_size*NR_POLARIZATIONS +
        ...
        std::fill(&tiles[tile_ids[i]*padded_tile_size*padded_tile_size*NR_POLARIZATIONS],
                  &tiles[(tile_ids[i]+1)*padded_tile_size*padded_tile_size*NR_POLARIZATIONS],
                  idg::float2({0.0, 0.0}));

        idg::ifft2f(NR_POLARIZATIONS, w_padded_tile_size, w_padded_tile_size, (std::complex<float>*) tile_buffer.data());
        // multiply w term
        ...
        for (int y = 0; y < w_padded_tile_size; y++) {
            for (int x = 0; x < w_padded_tile_size; x++) {
                ...
                // Apply correction
                tile_buffer[                                          y*w_padded_tile_size + x] = tile_buffer[                                          y*w_padded_tile_size + x] * phasor;
                tile_buffer[  w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] = tile_buffer[  w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] * phasor;
                tile_buffer[2*w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] = tile_buffer[2*w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] * phasor;
                tile_buffer[3*w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] = tile_buffer[3*w_padded_tile_size*w_padded_tile_size + y*w_padded_tile_size + x] * phasor;
            }
        }

        idg::fft2f(NR_POLARIZATIONS, w_padded_tile_size, w_padded_tile_size, (std::complex<float>*) tile_buffer.data());
        ...
        for (int y = y_start; y < y_end; y++) {
            for (int x = x_start; x < x_end; x++) {
                grid[index_grid(grid_size,0,y,x)] += tile_buffer[                                          (y-y0)*w_padded_tile_size + x-x0];
                grid[index_grid(grid_size,1,y,x)] += tile_buffer[  w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0];
                grid[index_grid(grid_size,2,y,x)] += tile_buffer[2*w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0];
                grid[index_grid(grid_size,3,y,x)] += tile_buffer[3*w_padded_tile_size*w_padded_tile_size + (y-y0)*w_padded_tile_size + x-x0];
            }
        }
    }
