...
extern "C" {
void kernel_adder(
    const unsigned long  nr_subgrids,
          ...
          idg::float2*   grid)
{
    ...
    for (unsigned row = 0; row < grid_size; row++) {
        for (unsigned s = 0; s < nr_subgrids; s++) {
            ...
            unsigned subgrid_x = metadata[s].coordinate.x;
            unsigned subgrid_y = metadata[s].coordinate.y;
            ...
            if (subgrid_x >= 0 && subgrid_x < grid_size-subgrid_size &&
                subgrid_y >= 0 && subgrid_y < grid_size-subgrid_size &&
                 offset_y >= 0 &&  offset_y < (int) subgrid_size) {

                // Iterate all columns of subgrid
                for (unsigned x = 0; x < subgrid_size; x++) {
                    // Compute shifted position in subgrid
                    unsigned long x_src = (x + (subgrid_size/2)) % subgrid_size;
                    unsigned long y_src = (offset_y + (subgrid_size/2)) % subgrid_size;
