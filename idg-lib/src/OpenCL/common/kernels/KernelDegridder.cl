    ...
    for (int i = tidx; i < ALIGN(nr_timesteps * nr_channels, nr_threads); i += nr_threads) {
        ...
        for (int pixel_offset = 0; pixel_offset < nr_pixels; pixel_offset += current_nr_pixels) {
            ...
            for (int i = tid; i < current_nr_pixels; i += nr_threads) {
                ...
                // Store pixels
                _pix[0][i] = (float4) (pixelsXX.x, pixelsXX.y, pixelsXY.x, pixelsXY.y);
                _pix[1][i] = (float4) (pixelsYX.x, pixelsYX.y, pixelsYY.x, pixelsYY.y);
                ...
                _lmn_phaseoffset[i] = (float4) (l, m, n, phase_offset);
            }
            ...
            for (int j = 0; j < current_nr_pixels; j++) {
                ...
                float2 apXX = (float2) (_pix[0][j].x, _pix[0][j].y);
                float2 apXY = (float2) (_pix[0][j].z, _pix[0][j].w);
                float2 apYX = (float2) (_pix[1][j].x, _pix[1][j].y);
                float2 apYY = (float2) (_pix[1][j].z, _pix[1][j].w);
