    ...
    for (int i = tidx; i < ALIGN(nr_timesteps * current_nr_channels, NR_THREADS); i += NR_THREADS) {
		...
		for (int j = tidx; j < ALIGN(SUBGRIDSIZE * SUBGRIDSIZE, NR_THREADS); j += NR_THREADS) {
            ...
            if (y < SUBGRIDSIZE) {
                ...
                // Load spheroidal
                float _spheroidal = spheroidal[y * SUBGRIDSIZE + x];
                ...
                // Load uv values
                int idx_xx = index_subgrid(SUBGRIDSIZE, s, 0, y_src, x_src);
                int idx_xy = index_subgrid(SUBGRIDSIZE, s, 1, y_src, x_src);
                int idx_yx = index_subgrid(SUBGRIDSIZE, s, 2, y_src, x_src);
                int idx_yy = index_subgrid(SUBGRIDSIZE, s, 3, y_src, x_src);
