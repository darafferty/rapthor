...
class InstanceOpenCL : public KernelsInstance {
 public:
  // Constructor
  InstanceOpenCL(cl::Context& context, int device_nr, int device_id);
  ...
  void launch_gridder(int nr_timesteps, int nr_subgrids, int grid_size,
                      int subgrid_size, float image_size, float w_step,
                      int nr_channels, int nr_stations, cl::Buffer& d_uvw,
                      cl::Buffer& d_wavenumbers, cl::Buffer& d_visibilities,
                      cl::Buffer& d_spheroidal, cl::Buffer& d_aterm,
                      cl::Buffer& d_metadata, cl::Buffer& d_subgrid);

  void launch_degridder(int nr_timesteps, int nr_subgrids, int grid_size,
                        int subgrid_size, float image_size, float w_step,
                        int nr_channels, int nr_stations, cl::Buffer& d_uvw,
                        cl::Buffer& d_wavenumbers, cl::Buffer& d_visibilities,
                        cl::Buffer& d_spheroidal, cl::Buffer& d_aterm,
  ...
  void plan_fft(unsigned size, unsigned batch);

  void launch_fft(cl::Buffer& d_data, DomainAtoDomainB direction);

  void launch_adder(int nr_subgrids, int grid_size, int subgrid_size,
                    cl::Buffer& d_metadata, cl::Buffer& d_subgrid,
                    cl::Buffer& d_grid);

  void launch_splitter(int nr_subgrids, int grid_size, int subgrid_size,
                       cl::Buffer& d_metadata, cl::Buffer& d_subgrid,
                       cl::Buffer& d_grid);

  void launch_scaler(int nr_subgrids, int subgrid_size, cl::Buffer& d_subgrid);

 protected:
  void compile_kernel_gridder(unsigned nr_channels);
  void compile_kernel_degridder(unsigned nr_channels);
  cl::Kernel* compile_kernel(int kernel_id, std::string file_name,
 ...
 public:
  // Memory management
  cl::Buffer& get_device_grid(unsigned int grid_size = 0);

  cl::Buffer& get_device_wavenumbers(unsigned int nr_channels = 0);

  cl::Buffer& get_device_aterms(unsigned int nr_stations = 0,
                                unsigned int nr_timeslots = 0,
                                unsigned int subgrid_size = 0);

  cl::Buffer& get_device_spheroidal(unsigned int subgrid_size = 0);

  cl::Buffer& get_host_grid(unsigned int grid_size = 0);

  cl::Buffer& get_host_visibilities(unsigned int nr_baselines = 0,
                                    unsigned int nr_timesteps = 0,
                                    unsigned int nr_channels = 0,
                                    void* ptr = NULL);

  cl::Buffer& get_host_uvw(unsigned int nr_baselines = 0,
                           unsigned int nr_timesteps = 0, void* ptr = NULL);

 public:
  powersensor::State measure();
  void measure(PowerRecord& record, cl::CommandQueue& queue);

  void enqueue_report(cl::CommandQueue& queue, int nr_timesteps,
                      int nr_subgrids);
 ...
 private:
  cl::Buffer* reuse_memory(uint64_t size, cl::Buffer* buffer,
                           cl_mem_flags flags, void* ptr = NULL);
