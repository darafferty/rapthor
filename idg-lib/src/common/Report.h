...
inline const std::string get_name(ID id){
  switch (id)
  {
    case gridding     : return auxiliary::name_gridding;
    case degridding   : return auxiliary::name_degridder;
    case adding       : return auxiliary::name_adding;
    case splitting    : return auxiliary::name_splitting;
    case adder        : return auxiliary::name_adder;
    case splitter     : return auxiliary::name_splitter;
    case gridder      : return auxiliary::name_gridder;
    case degridder    : return auxiliary::name_degridder;
    case calibrate    : return auxiliary::name_calibrate;
    case subgrid_fft  : return auxiliary::name_subgrid_fft;
    case grid_fft     : return auxiliary::name_grid_fft;
    case fft_shift    : return auxiliary::name_fft_shift;
    case fft_scale    : return auxiliary::name_fft_scale;
    case average_beam : return auxiliary::name_average_beam;
    case wtiling      : return auxiliary::name_wtiling;
    case host         : return auxiliary::name_host;
    case device       : return auxiliary::name_device;
...
inline const uint64_t get_flops(ID id, Parameters parameters){
  ...
  switch (id)
  {
    case adder        : return auxiliary::flops_adder(nr_subgrids, subgrid_size);
    case splitter     : return auxiliary::flops_splitter(nr_subgrids, subgrid_size);
    case gridder      : return auxiliary::flops_gridder(nr_channels, nr_timesteps, nr_subgrids, subgrid_size);
    case degridder    : return auxiliary::flops_degridder(nr_channels, nr_timesteps, nr_subgrids, subgrid_size);
    case calibrate    : return auxiliary::flops_calibrate(nr_terms, nr_channels, nr_timesteps, nr_subgrids, subgrid_size);
    case subgrid_fft  : return auxiliary::flops_fft(subgrid_size, nr_subgrids);
    case grid_fft     : return auxiliary::flops_fft(grid_size, 1);
    default           : return 0;
  }
}
...
inline const uint64_t get_bytes(ID id, Parameters parameters){
  ...
  switch (id)
  {
    case adder        : return auxiliary::bytes_adder(nr_subgrids, subgrid_size);
    case splitter     : return auxiliary::bytes_splitter(nr_subgrids, subgrid_size);
    case gridder      : return auxiliary::bytes_gridder(nr_channels, nr_timesteps, nr_subgrids, subgrid_size);
    case degridder    : return auxiliary::bytes_degridder(nr_channels, nr_timesteps, nr_subgrids, subgrid_size);
    case subgrid_fft  : return auxiliary::bytes_fft(subgrid_size, nr_subgrids);
    case grid_fft     : return auxiliary::bytes_fft(grid_size, 1);
    default           : return 0;
  }
}
...
public:
 ...
 public:
  ...
  template<ID id>
  void update(powersensor::State& start,
              powersensor::State& end)
  {
    update(id, start, end);
  }
  ...
  void print(int nr_timesteps, int nr_subgrids, bool total = false,
    ...
    for (auto& item : items) {
      if ((total && item.enabled) || item.updated)
      {
        ...
        report(prefix + get_name(item.id), seconds, joules, flops, bytes, ignore_short);
        item.updated = false;
      }
    }
  }
