...
class Report {
 ...
 public:
  ...
  void update(Report::State& reportState, double runtime) {
    reportState.current_seconds = runtime;
    reportState.total_seconds += reportState.current_seconds;
  }

  void update(Report::State& reportState, powersensor::State& startState,
              powersensor::State& endState);

  void update_host(powersensor::State& startState,
                   powersensor::State& endState) {
    host_enabled = true;
    host_updated = true;
  ...
  void update_gridder(powersensor::State& startState,
                      powersensor::State& endState) {
  ...
  void update_gridder_post(powersensor::State& startState,
                           powersensor::State& endState) {
  ...
  void update_degridder(powersensor::State& startState,
                        powersensor::State& endState) {
  ...
  void update_degridder_pre(powersensor::State& startState,
                            powersensor::State& endState) {
  ...
  void update_calibrate(powersensor::State& startState,
                        powersensor::State& endState) {
  ...
  void update_adder(powersensor::State& startState,
                    powersensor::State& endState) {
    adder_enabled = true;
    adder_updated = true;
  ...
  void update_splitter(powersensor::State& startState,
                       powersensor::State& endState) {
  ...
  void update_scaler(powersensor::State& startState,
                     powersensor::State& endState) {
    scaler_enabled = true;
    scaler_updated = true;
  ...
  void update_subgrid_fft(powersensor::State& startState,
                          powersensor::State& endState) {
  ...
  void update_grid_fft(powersensor::State& startState,
                       powersensor::State& endState) {
  ...
  void update_fft_shift(powersensor::State& startState,
                        powersensor::State& endState) {
  ...
  void update_fft_shift(double runtime) {
    fft_shift_enabled = true;
    fft_shift_updated = true;
    update(state_fft_shift, runtime);
  }

  void update_fft_scale(powersensor::State& startState,
                        powersensor::State& endState) {
  ...
  void update_fft_scale(double runtime) {
    fft_scale_enabled = true;
    fft_scale_updated = true;
  ...
  void update_output(powersensor::State& startState,
                     powersensor::State& endState) {
    output_enabled = true;
    output_updated = true;
  ...
  void update_device(powersensor::State& startState,
                     powersensor::State& endState, unsigned id = 0) {
  ...
  void update_devices(std::vector<powersensor::State> start,
  ...
  void print(int nr_timesteps, int nr_subgrids, bool total = false,
    ...
    if ((total && gridder_enabled) || gridder_updated) {
      ...
      if (gridder_post_updated) {
        seconds += total ? state_gridder_post.total_seconds
                         : state_gridder_post.current_seconds;
        joules += total ? state_gridder_post.total_joules
                        : state_gridder_post.current_joules;
      }
      report(prefix + auxiliary::name_gridder, seconds, joules,
             auxiliary::flops_gridder(nr_channels, nr_timesteps, nr_subgrids,
                                      subgrid_size),
             auxiliary::bytes_gridder(nr_channels, nr_timesteps, nr_subgrids,
                                      subgrid_size),
             ignore_short);
      gridder_updated = false;
    }
    if ((total && degridder_enabled) || degridder_updated) {
      auto seconds = total ? state_degridder.total_seconds
                           : state_degridder.current_seconds;
      auto joules =
          total ? state_degridder.total_joules : state_degridder.current_joules;
      if (degridder_pre_updated) {
        seconds += total ? state_degridder_pre.total_seconds
                         : state_degridder_pre.current_seconds;
        joules += total ? state_degridder_pre.total_joules
                        : state_degridder_pre.current_joules;
      }
      report(prefix + auxiliary::name_degridder, seconds, joules,
             auxiliary::flops_degridder(nr_channels, nr_timesteps, nr_subgrids,
                                        subgrid_size),
             auxiliary::bytes_degridder(nr_channels, nr_timesteps, nr_subgrids,
                                        subgrid_size),
             ignore_short);
      degridder_updated = false;
    }
    if ((total && subgrid_fft_enabled) || subgrid_fft_updated) {
      report(prefix + auxiliary::name_subgrid_fft,
             total ? state_subgrid_fft.total_seconds
                   : state_subgrid_fft.current_seconds,
             total ? state_subgrid_fft.total_joules
                   : state_subgrid_fft.current_joules,
             auxiliary::flops_fft(subgrid_size, nr_subgrids),
             auxiliary::bytes_fft(subgrid_size, nr_subgrids), ignore_short);
      subgrid_fft_updated = false;
    }
    if ((total && calibrate_enabled) || calibrate_updated) {
      report(
          prefix + auxiliary::name_calibrate,
          total ? state_calibrate.total_seconds
                : state_calibrate.current_seconds,
          total ? state_calibrate.total_joules : state_calibrate.current_joules,
          auxiliary::flops_calibrate(nr_terms, nr_channels, nr_timesteps,
                                     nr_subgrids, subgrid_size),
          auxiliary::bytes_calibrate(), ignore_short);
      calibrate_updated = false;
    }
    if ((total && adder_enabled) || adder_updated) {
      report(prefix + auxiliary::name_adder,
             ...
             auxiliary::bytes_adder(nr_subgrids, subgrid_size), ignore_short);
      adder_updated = false;
    }
    if ((total && splitter_enabled) || splitter_updated) {
      report(
          ...
          auxiliary::bytes_splitter(nr_subgrids, subgrid_size), ignore_short);
      splitter_updated = false;
    }
    if ((total && scaler_enabled) || scaler_updated) {
      report(prefix + auxiliary::name_scaler,
             ...
             auxiliary::bytes_scaler(nr_subgrids, subgrid_size), ignore_short);
    ...
    if ((total && grid_fft_updated) || grid_fft_updated) {
      report(
          ...
          auxiliary::bytes_fft(grid_size, 1), ignore_short);
      grid_fft_updated = false;
    }
    if ((total && fft_shift_enabled) || fft_shift_updated) {
      report(prefix + auxiliary::name_fft_shift,
             total ? state_fft_shift.total_seconds
                   : state_fft_shift.current_seconds,
             0, 0, 0, ignore_short);
      fft_shift_updated = false;
    }
    if ((total && fft_scale_enabled) || fft_scale_updated) {
      report(prefix + auxiliary::name_fft_scale,
             total ? state_fft_scale.total_seconds
                   : state_fft_scale.current_seconds,
             0, 0, 0, ignore_short);
    ...
    if ((total && host_enabled) || host_updated) {
      report(prefix + auxiliary::name_host,
             ...
             total ? state_host.total_joules : state_host.current_joules, 0, 0,
             ignore_short);
      host_updated = false;
    }
  }
  ...
  void print_visibilities(const std::string name, int nr_visibilities = 0) {
    ...
    report_visibilities(prefix + name, state_host.total_seconds,
                        nr_visibilities);
  }

  void print_device(powersensor::State& startState,
                    powersensor::State& endState, int i = -1);

  void print_devices(std::vector<powersensor::State> start,
                     std::vector<powersensor::State> end);
