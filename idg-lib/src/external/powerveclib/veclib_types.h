...
/* Control inlining */
#ifdef NEVER_INLINE
  #define VECLIB_INLINE static __attribute__ ((__noinline__))
#else
...
typedef
  ...
  union {
    ...
    float                     as_float             [4];
    int                       as_hex               [4];
    unsigned int              as_hex_unsigned      [4];
  } __m128_union;
...
typedef
  ...
  typedef
    ...
    union {
	  ...
	  unsigned long long      as_unsigned_long_hex [2];
	  vector unsigned long long as_vector_unsigned_long_long;
    } __m128d_union;
#endif
...
typedef
  ...
  union {
    ...
    float                     as_float             [8];
    int                       as_hex               [8];
    unsigned int              as_hex_unsigned      [8];
  } __m256_union;
