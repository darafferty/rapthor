        ...
        for fname in sub_dcmp.common_files:
            ...
            if "fits" in ext:
                if "image" in dname or "mosaic" in dname:
                    if not image_files_are_similar(left, right, rtol=rtol, verbosity=verbosity):
                        agree = False
                else:
                    if not fits_files_are_similar(left, right, rtol=rtol, verbosity=verbosity):
                        agree = False
            if "h5" in ext:
                if not h5_files_are_similar(left, right, rtol=rtol, verbosity=verbosity):
                    agree = False
...
def parse_arguments():
    ...
    data_group.add_argument("--images", action="store_true", help="Compare saved images only")
    data_group.add_argument("--skymodels", action="store_true", help="Compare saved skymodels only")
    data_group.add_argument("--solutions", action="store_true", help="Compare saved solutions only")
    data_group.add_argument(
...
def main(args):
    ...
    if args.images or args.all or no_opts:
        if not compare_results(
            filecmp.dircmp(os.path.join(args.first, "images"), os.path.join(args.second, "images")),
            atol=args.atol,
            rtol=args.rtol,
            verbosity=verbosity,
        ):
