...
void wtiles_to_grid(int grid_size, int subgrid_size, int wtile_size,
        ...
        for (int x = 0; x < padded_tile_size; x++) {
          ...
          tile_buffer[index_grid(w_padded_tile_size, index_pol_transposed[pol],
              ...
              reinterpret_cast<std::complex<float>*>(
                  tiles)[index_grid(padded_tile_size, tile_ids[i], pol, y, x)];
        }
      }
    }
      ...
      for (int x = x_start; x < x_end; x++) {
        for (int pol = 0; pol < NR_CORRELATIONS; pol++) {
          grid[index_grid(grid_size, pol, y, x)] +=
              tile_buffer
                  .data()[index_grid(w_padded_tile_size, pol, y - y0, x - x0)];
        }
      }
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  unsigned batch = nr_tiles * NR_CORRELATIONS;
  ...
  cuda.launch_adder_apply_phasor(nr_tiles, image_size, w_step, padded_tile_size,
                                 d_padded_tiles, d_shift, d_tile_coordinates);
  ...
  cuda.launch_adder_wtiles_to_grid(nr_tiles, tile_size - subgrid_size,
  ...
  grid.zero();
  wtiles_to_grid(grid_size, subgrid_size, tile_size - subgrid_size, image_size,
                 w_step, shift.data(), nr_tiles, tile_ids.data(),
                 tile_coordinates.data(), h_tiles, grid.data());
