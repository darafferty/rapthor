...
__kernel void kernel_degridder_8(
    ...
    for (int i = tid; i < nr_timesteps * nr_channels; i += NR_THREADS) {
        ...
        int vis_offset = ((time_offset_global + time) * nr_channels) + channel_offset;
        if (time < nr_timesteps) {
            visibilities[vis_offset + chan][0] = (float2) (vis.s0, vis.s1);
            visibilities[vis_offset + chan][1] = (float2) (vis.s2, vis.s3);
            visibilities[vis_offset + chan][2] = (float2) (vis.s4, vis.s5);
            visibilities[vis_offset + chan][3] = (float2) (vis.s6, vis.s7);
		}
	}
}
