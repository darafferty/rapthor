...
void PowerSensor::doMeasurement()
{
  ...
  if (lastState.microSeconds != currentState.microSeconds) {
    ...
    if (dumpFile != 0)
      *dumpFile << "S " << currentState.microSeconds / 1e6 << ' ' << (currentState.consumedEnergy - previousState.consumedEnergy) * (65536.0 / 511) / (currentState.microSeconds - previousState.microSeconds) << std::endl;
  }
...
double PowerSensor::Watt(const State &firstState, const State &secondState)
{
  ...
  if (microSeconds != 0)
    return (secondState.lastState.consumedEnergy - firstState.lastState.consumedEnergy) * (65536.0 / 511) / microSeconds;
  else // very short time
    return (secondState.lastState.consumedEnergy - secondState.previousState.consumedEnergy) * (65536.0 / 511) / (secondState.lastState.microSeconds - secondState.previousState.microSeconds);
}
