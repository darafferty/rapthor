...
{% endif %}
  ...
  - id: check_beam_true_sky_image
    label: Check beam
    doc: |
      This step checks that the restoring beam in the true-sky image is valid. Valid
      beam information is needed for the later filter and find_diagnostics steps but is
      not generated by WSClean when clean is disabled.
...
{% endif %}
  ...
  - id: check_beam_flat_noise_image
    label: Check beam
    doc: |
      This step checks that the restoring beam in the flat-noise image is valid. Valid
      beam information is needed for the later filter and find_diagnostics steps but is
      not generated by WSClean when clean is disabled..
    run: {{ rapthor_pipeline_dir }}/steps/check_image_beam.cwl
    in:
...
{% endif %}
  ...
  - id: make_skymodel_image
    ...
    in:
      ...
      - id: reference_image
        source: filter/skymodel_image_fits
      - id: output_image_name
        source: filter/skymodel_image_fits
        valueFrom: $(self.basename) + ".fits.fz"
    when: $(inputs.save_model_image == true)
    out:
      - id: output_image
  - id: find_diagnostics
