...
baseCommand: [DP3]
id: idgcal_solve_phase_and_gain
label: Calibrates a dataset using IDGCal
doc: |
  This tool solves for phase and amplitude screens using IDGCal for the given MS
  file, using the input model image. Output is the solution table in h5parm
  format.
...
inputs:
  ...
  - id: solint_fast
    label: Fast solution interval
    doc: |
      The solution interval in timeslots for the fast-phase solve.
    type: int
    inputBinding:
  ...
  - id: solint_slow
    label: Slow solution interval
    doc: |
      The solution interval in timeslots for the slow-gain solve.
    type: int
    inputBinding:
      prefix: solve.solintamplitude=
      separate: False
