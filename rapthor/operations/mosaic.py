...
class MosaicCal(Operation):
    ...
    def set_input_parameters(self):
        ...
        """
        # First, determine whether processing is needed
        if len(self.field.cal_sectors) > 1:
            skip_processing = False
        else:
            # No need to mosaic if we have just one sector
            skip_processing = True
    ...
    def finalize(self):
        ...
        # Save the FITS image and model
        dst_dir = os.path.join(self.field.parset['dir_working'], 'images',
                               'cal_image_{}'.format(self.index))
        misc.create_directory(dst_dir)
#        self.field.field_image_filename_prev = self.field.field_image_filename
        self.field.field_cal_image_filename = os.path.join(dst_dir, 'field-MFS-I-image.fits')
        os.system('cp {0} {1}'.format(self.mosaic_filename, self.field.field_cal_image_filename))
