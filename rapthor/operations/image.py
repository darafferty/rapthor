...
class ImageCal(Operation):
    ...
    def set_input_parameters(self):
        ...
        Define the pipeline inputs
        """
        nsectors = len(self.field.cal_sectors)
        obs_filename = []
        prepare_filename = []
        previous_mask_filename = []
        mask_filename = []
        ...
        self.input_parms = {'obs_filename': obs_filename,
                            ...
                            'dir_local': dir_local,
                            'do_slowgain_solve': [self.field.do_slowgain_solve] * nsectors,
                            'channels_out': [sector.wsclean_nchannels for sector in self.field.cal_sectors],
                            'deconvolution_channels': [sector.wsclean_deconvolution_channels for sector in self.field.cal_sectors],
                            'ra': [sector.ra for sector in self.field.cal_sectors],
                            'dec': [sector.dec for sector in self.field.cal_sectors],
                            'wsclean_imsize': [sector.imsize for sector in self.field.cal_sectors],
                            'vertices_file': [sector.vertices_file for sector in self.field.cal_sectors],
                            'region_file': [sector.region_file for sector in self.field.cal_sectors],
                            'wsclean_niter': [sector.wsclean_niter for sector in self.field.cal_sectors],
                            'wsclean_nmiter': [sector.wsclean_nmiter for sector in self.field.cal_sectors],
                            'robust': [sector.robust for sector in self.field.cal_sectors],
                            'cellsize_deg': [sector.cellsize_deg for sector in self.field.cal_sectors],
                            'min_uv_lambda': [sector.min_uv_lambda for sector in self.field.cal_sectors],
                            'max_uv_lambda': [sector.max_uv_lambda for sector in self.field.cal_sectors],
                            'taper_arcsec': [sector.taper_arcsec for sector in self.field.cal_sectors],
                            'auto_mask': [sector.auto_mask for sector in self.field.cal_sectors],
                            'idg_mode': [sector.idg_mode for sector in self.field.cal_sectors],
                            'wsclean_mem': [sector.mem_percent for sector in self.field.cal_sectors],
    ...
    def finalize(self):
        ...
        # NOTE: currently, -save-source-list only works with pol=I -- when it works with other
        # pols, save them all
        for sector in self.field.cal_sectors:
            image_root = os.path.join(self.pipeline_working_dir, sector.name, sector.name)
            sector.I_image_file_true_sky = image_root + '-MFS-image-pb.fits'
            sector.I_image_file_apparent_sky = image_root + '-MFS-image.fits'
            sector.I_model_file_true_sky = image_root + '-MFS-model-pb.fits'
