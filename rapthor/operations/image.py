...
class Image(Operation):
    ...
    def finalize(self):
        ...
        for index, sector in enumerate(self.field.imaging_sectors):
            ...
            # supported
            if self.make_image_cube:
                src_filename = self.outputs["sector_image_cube"][index]
                filename = f'{image_root}_freq_cube.fits'
                dst_dir = os.path.join(self.parset['dir_working'], 'images', self.name)
                os.makedirs(dst_dir, exist_ok=True)
                sector.I_freq_cube = os.path.join(dst_dir, os.path.basename(filename))
                shutil.copy(src_filename, sector.I_freq_cube)

                for src_filename in [self.outputs["sector_image_cube_beams"][index],
            ...
            if self.field.image_pol.lower() == 'i':
                ...
                for src_filename, filename in zip(
                                    [self.outputs["sector_skymodel_true_sky"][index],
                                     self.outputs["sector_skymodel_apparent_sky"][index]],
            ...
            if self.use_facets:
                ...
                os.makedirs(dst_dir, exist_ok=True)
                src_filename = self.outputs["sector_region_file"][index]
            ...
            if self.field.save_visibilities:
                ...
                ms_filenames = self.outputs["visibilities"][index]
