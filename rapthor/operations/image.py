...
class Image(Operation):
    ...
    def __init__(self, field, index, name="image"):
        ...
        self.apply_none = (
            False  # no solutions applied before or during imaging (ImageInitial only)
        )
        ...
        def find_in_file_list(file_list):
            ...
            for name, ext in ext_mapping.items():
                for filename in file_list:
                    if ext in filename:
                        type_path_map.setdefault(name, []).append(filename)
        ...
        for index, sector in enumerate(self.field.imaging_sectors):
            ...
            if self.field.image_pol.lower() == "i":
                for skymodel_type in ["true_sky", "apparent_sky"]:
                    ...
                    setattr(
                        sector,
                        f"image_skymodel_file_{skymodel_type}",
                        sector_skymodel_file,
                    )
            ...
            dst_dir = os.path.join(
                self.parset["dir_working"], "image_{}".format(self.index), sector.name
            )
...
class ImageInitial(Image):
    ...
    def finalize(self):
        ...
        self.copy_outputs_to(
            os.path.join(self.parset["dir_working"], self.name, sector.name),
...
def report_sector_diagnostics(sector_name, diagnostics_dict, log):
    ...
    try:
        ...
        if (
            ...
            log.info(
                "    LOFAR-PanSTARRS RA offset = {0} +/- {1}".format(raoff, stdraoff)
            )
        else:
        ...
        if (
            ...
            log.info(
                "    LOFAR-PanSTARRS Dec offset = {0} +/- {1}".format(decoff, stddecoff)
            )
        else:
