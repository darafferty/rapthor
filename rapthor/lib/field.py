...
class Field(object):
    ...
    def __init__(self, parset, minimal=False):
        ...
        self.medium_smoothnessconstraint = self.parset["calibration_specific"][
            "medium_smoothnessconstraint"
        ]
        self.medium_smoothnessreffrequency = self.parset["calibration_specific"][
            "medium_smoothnessreffrequency"
        ]
        self.medium_smoothnessrefdistance = self.parset["calibration_specific"][
            "medium_smoothnessrefdistance"
        ]
        self.slow_smoothnessconstraint = self.parset["calibration_specific"][
        ...
        self.normalization_reference_frequencies = self.parset["imaging_specific"][
        ...
        self.correct_smearing_in_calibration = self.parset["calibration_specific"][
            "correct_time_frequency_smearing"
        ]
        self.correct_smearing_in_imaging = self.parset["imaging_specific"][
            "correct_time_frequency_smearing"
        ]
        self.cycle_number = 1
    ...
    def scan_observations(self):
        ...
        self.fwhm_deg = (
            1.1 * ((3.0e8 / np.mean(ref_freq_list)) / self.diam) * 180.0 / np.pi * sec_el
        )
    ...
    def make_skymodels(
        ...
        if type(skymodel_true_sky) is not lsmtool.skymodel.SkyModel:
            skymodel_true_sky = lsmtool.load(str(skymodel_true_sky), beamMS=self.beam_ms_filename)
        ...
        if regroup:
            if (facets := self.read_facet_layout_from_file()) is not None:
                ...
                if target_flux is not None and target_flux <= 0.0:
                    raise ValueError("The target flux density cannot be less than or equal to 0.")
                ...
                if target_number is not None:
                    ...
                    if target_flux is None:
                        ...
                        if target_flux_for_number > target_flux and target_number < len(fluxes):
                            ...
                            self.log.info(
                                "Using a target flux density of %.2f Jy for grouping", target_flux
                            )
                else:
                    self.log.info(
                        "Using a target flux density of %.2f Jy for grouping", target_flux
    ...
    def update_skymodels(
        ...
        if index == 1:
            ...
            self.plot_overview("initial_field_overview.png", show_initial_coverage=True, moc=moc)
            ...
            sector_skymodels_apparent_sky = [
                sector.image_skymodel_file_apparent_sky for sector in self.imaging_sectors
            ]
            ...
            if combine_current_and_intial or not self.imaged_sources_only:
                ...
                for sector in self.imaging_sectors:
                    skymodel_true_sky_start = sector.filter_skymodel(
                        skymodel_true_sky_start, invert=True
                    )
        ...
        for sector in self.imaging_sectors:
            ...
            sector.make_region_file(
                os.path.join(self.working_dir, "regions", f"{sector.name}_region_ds9.reg")
            )
    ...
    def define_imaging_sectors(self):
        ...
        if len(self.parset["imaging_specific"]["sector_center_ra_list"]) > 0:
            ...
            if nsectors_ra == 1 and nsectors_dec == 1:
                ...
                center_x, center_y = self.wcs.wcs_world2pix(
                    [image_ra], [image_dec], misc.WCS_ORIGIN
                )
                x = np.array([center_x])
                y = np.array([center_y])
            else:
    ...
    def define_sector_bounds(self):
        ...
        max_ra, min_dec = self.wcs.wcs_pix2world(
            self.sector_bounds_xy[0], self.sector_bounds_xy[1], misc.WCS_ORIGIN
        )
        min_ra, max_dec = self.wcs.wcs_pix2world(
            self.sector_bounds_xy[2], self.sector_bounds_xy[3], misc.WCS_ORIGIN
        )
        mid_ra, mid_dec = self.wcs.wcs_pix2world(
        ...
        self.sector_bounds_width_ra = abs(
            (self.sector_bounds_xy[0] - self.sector_bounds_xy[2]) * self.wcs.wcs.cdelt[0]
        )
        self.sector_bounds_width_dec = abs(
            (self.sector_bounds_xy[3] - self.sector_bounds_xy[1]) * self.wcs.wcs.cdelt[1]
        )
            ...
            if nsources > 0:
                ...
                for i in range(nsectors):
                    bright_source_sector = Sector(
                        f"bright_source_{i + 1}", self.ra, self.dec, 1.0, 1.0, self
                    )
    ...
    def define_full_field_sector(self, radius=None):
        ...
        if radius is None:
            ...
            width_ra = radius * 2
            width_dec = radius * 2
        self.full_field_sector = Sector("full_field", self.ra, self.dec, width_ra, width_dec, self)

        # Make sector region and vertices files
        self.full_field_sector.make_vertices_file()
        self.full_field_sector.make_region_file(
            os.path.join(
                self.working_dir, "regions", f"{self.full_field_sector.name}_region_ds9.reg"
            )
        )
    ...
    def find_intersecting_sources(self):
        ...
        if len(intersecting_ind) > 0:
            ...
            intersecting_source_polys = [
                Point(xp, yp).buffer(sp * 1.5) for xp, yp, sp in zip(xfilt, yfilt, sfilt)
            ]
        else:
            intersecting_source_polys = []
    ...
    def make_outlier_skymodel(self, threshold_ratio=0.001, threshold_jy=0.1):
        ...
        outlier_ind = np.array(
            [all_source_names.index(sn) for sn in all_source_names if sn not in sector_source_names]
        )
    ...
    def check_selfcal_progress(self):
        ...
        if convergence_ratio > 2.0:
            self.log.warning(
                "The convergence ratio is set to %.2f but must be <= 2. Using 2.0 instead",
                convergence_ratio,
            )
            convergence_ratio = 2.0
        if convergence_ratio < 0.5:
            self.log.warning(
                "The convergence ratio is set to %.2f but must be >= 0.5. Using 0.5 instead",
                convergence_ratio,
            )
            convergence_ratio = 0.5
        if divergence_ratio < 1.0:
            self.log.warning(
                "The divergence ratio is set to %.2f but must be >= 1. Using 1.0 instead",
                divergence_ratio,
            )
            divergence_ratio = 1.0
        if failure_ratio < 1.0:
            self.log.warning(
                "The failure ratio is set to %.2f but must be >= 1. Using 1.0 instead",
                failure_ratio,
            )
            failure_ratio = 1.0
    ...
    def get_matplotlib_patch(self, wcs=None):
        ...
        if wcs is None:
            ...
            # two axes
            wcs_pixel_scale = (
                wcs.proj_plane_pixel_scales()[0].value + wcs.proj_plane_pixel_scales()[1].value
            ) / 2
    ...
    def plot_overview(
        ...
        if self.parset["generate_initial_skymodel"]:
            initial_skymodel_radius = (
                max(self.full_field_sector.width_ra, self.full_field_sector.width_dec) / 2
            )
        elif self.parset["download_initial_skymodel"]:
        ...
        for i, sector in enumerate(self.imaging_sectors):
            ...
            xy_sector = wcs.wcs_world2pix(
                sector.ra, sector.dec + sector.width_dec / 2, misc.WCS_ORIGIN
            )  # center-top
