...
class Parset:
    ...
    def __check_and_adjust(self, settings):
        ...
        if options["use_image_based_predict"] and options["use_wsclean_predict"]:
            log.warning(
                "Options use_image_based_predict and use_wsclean_predict are mutually exclusive. Turning off image_based_predict"
            )
        ...
        if options["save_residual_visibilities"] and (
            options["bda_timebase"] > 0
        ):
            log.warning("Switching off BDA during imaging, since residual visibilites are to be generated.")
            options["bda_timebase"] = 0
