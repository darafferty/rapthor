...
def set_user_strategy(field):
    ...
    try:
        ...
        raise ValueError(f"Strategy {field.parset['strategy']!r} does not define strategy_steps.")
...
def validate_strategy(strategy_steps, parset):
    ...
    if (
        not parset["cluster_specific"]["allow_internet_access"]
        and strategy_steps[0].get("do_normalize", False)
        and not parset["imaging_specific"]["normalization_skymodels"]
    ):
        raise ValueError(
