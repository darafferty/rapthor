...
class MainLogParser:
    ...
    def group_by_cycle(self, cycledict: dict) -> dict:
        ...
        for i, k in enumerate(cycle_keys, start=1):
            self.add_key(
                groupcycledict, f"cycle_{i:d}", f"calibrate_{i:d}", cycledict[f"calibrate_{i:d}"]
            )
            self.add_key(
                groupcycledict, f"cycle_{i:d}", f"predict_{i:d}", cycledict[f"predict_{i:d}"]
            )
            self.add_key(groupcycledict, f"cycle_{i:d}", f"image_{i:d}", cycledict[f"image_{i:d}"])
            self.add_key(
                groupcycledict, f"cycle_{i:d}", f"mosaic_{i:d}", cycledict[f"mosaic_{i:d}"]
            )
        ...
        with open("rapthor_timing.txt", "r") as f:
            ...
            for line in f.readlines():
                ...
                if "day" in timestr:
                    ...
                    dec_hour = dayhours + time.hour + time.minute / 60.0 + time.second / 3600.0
                else:
                    time = datetime.strptime(timestr, "%H:%M:%S.%f")
                    dec_hour = time.hour + time.minute / 60.0 + time.second / 3600.0
    ...
    def plot(self) -> None:
        ...
        h.set(
            xlabel="Self calibration cycle",
            ylabel="Duration [h]",
            title=f"Cumulative runtime: {df['Duration'].sum():.2f} hours",
        )
...
class SubLogParser:
    ...
    def __init__(self, logdir: str, operation: str) -> None:
        ...
        if not op_files:
            raise ValueError(
                'Could not find files named "CWLJob*.log". Pipeline was not run with Toil or all steps failed.'
            )
        ...
        for sublog in op_files:
            ...
            if "subpipeline" in sublog:
                sub_name = (
                    sublog.split("/")[-1]
                    .replace("CWLJob_subpipeline_parset.cwl.", "")
                    .split("--")[0]
                )
            elif "pipeline" in sublog:
                sub_name = (
                    sublog.split("/")[-1].replace("CWLJob_pipeline_parset.cwl.", "").split("--")[0]
                )
        ...
        if not self.run_times:
            ...
            for f in self.files:
                proc = subprocess.Popen(
                    ["grep", "we ran for a total of", f], stdout=subprocess.PIPE
                )
...
def make_cycle_pdfs_sublogs() -> None:
    ...
    try:
        ...
        cmd = (
            ["pdfunite", "rapthor_timing.pdf"]
            + sorted(glob.glob("summary_cycle_*.pdf"))
            + ["summary.pdf"]
        )
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detailed",
        help=(
            "Produce a detailed overview by breaking down each operation in "
            "its substeps. Requires Toil as the chosen CWL runner."
        ),
        action="store_true",
        required=False,
    )
