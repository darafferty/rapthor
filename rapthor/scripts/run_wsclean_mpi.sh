...
while getopts ":m:n:k:c:z:i:j:r:p:u:v:x:l:o:d:t:a:g:y:q:" arg; do
  case $arg in
    m) msin=$OPTARG;;
    n) name=$OPTARG;;
    k) mask=$OPTARG;;
...
mpi_command="mpirun -np \$SLURM_JOB_NUM_NODES -hostfile hostfile_${infix}.txt --pernode --prefix \$MPI_PREFIX -x LD_LIBRARY_PATH=\$LD_LIBRARY_PATH wsclean-mp -no-update-model-required -multiscale -fit-beam -reorder -save-source-list -local-rms -join-channels -use-idg -pol I -mgain 0.6 -log-time -fit-spectral-pol 3 -weighting-rank-filter 3 -auto-threshold 1.0 -local-rms-window 50 -local-rms-method rms-with-min -aterm-kernel-size 32 -weight briggs ${robust} -name ${name} -fits-mask ${mask} -aterm-config ${config} -size ${imsize} -niter ${niter} -nmiter ${nmiter} -padding ${padding} -minuv-l ${min_uv_lambda} -maxuv-l ${max_uv_lambda} -scale ${cellsize_deg} -parallel-deconvolution 2048 -temp-dir ${dir_local} -channels-out ${channels_out} -deconvolution-channels ${deconvolution_channels} -taper-gaussian ${taper_arcsec} -auto-mask ${auto_mask} -idg-mode ${idg_mode} ${msin}"
