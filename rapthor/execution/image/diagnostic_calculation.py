"""Image diagnostic calculation helpers."""
...
from lsmtool.facet import SquareFacet, read_ds9_region_file
...
def plot_astrometry_offsets(facets, field_ra, field_dec, output_file, plot_labels=False):
    ...
    facet_names = []
    for facet in facets:
        # Note: the offsets are calculated as (input model value) - (comparison
        # model value); e.g., a positive Dec offset indicates that the input
    ...
    # correction that should be applied to the input dource positions
...
def check_photometry(
    ...
    output_root : str or pathlib.Path
        Root of the filename for the output plots, including the output directory.
    ...
    #
    # Each comparison saves its plots with the output root and survey name.
    ...
    for survey, comparison_skymodel in comparison_skymodels.items():
        ...
        if statistics := compare_photometry_survey(
            catalog, survey, comparison_skymodel, freq, output_root
        ):
...
def compare_photometry_survey(catalog, survey, comparison_skymodel, freq, output_root):
    """
    ...
    # LSMTool uses fixed filenames, so isolate concurrent comparisons and discard
    # unneeded plots and stats.txt when the comparison finishes or fails.
    output_root = Path(output_root).resolve()
    with tempfile.TemporaryDirectory(
        dir=output_root.parent, prefix=f".{output_root.name}.photometry-"
    ) as plot_dir:
        if result := reference_skymodel.compare(
            ...
            _rename_plots(survey, Path(plot_dir), output_root)
    ...
    # Comparison failed due to insufficient matches.
    logger.info(
...
def _rename_plots(survey, filepath, output_root):
    """Move only the photometry plots to their output-root and survey filenames."""
    for plot in [
        "flux_ratio_vs_distance",
        "flux_ratio_vs_flux",
        "flux_ratio_sky",
    ]:
        (filepath / f"{plot}.pdf").replace(f"{output_root}.{plot}_{survey}.pdf")
