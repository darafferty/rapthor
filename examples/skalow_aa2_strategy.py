...
for i in range(max_selfcal_loops):
    ...
    strategy_steps[i]['do_slowgain_solve'] = False #let's not do amp self-cal until we can do phase self-cal
    if i == 0:
        strategy_steps[i]['peel_outliers'] = False #No outliers in the data
    else:
        strategy_steps[i]['peel_outliers'] = False
    strategy_steps[i]['solve_min_uv_lambda'] = 0
    ...
    strategy_steps[i]['fast_timestep_sec'] = 60.0 #solve every minute
    ...
    if i == 0:
        strategy_steps[i]['do_normalize'] = False #not doing amp self-cal
    else:
    ...
    if i == 0:
        strategy_steps[i]['auto_mask'] = 15.0 #image diverges for normal params!
        strategy_steps[i]['threshisl'] = 15.0
        strategy_steps[i]['threshpix'] = 5.0
        strategy_steps[i]['max_nmiter'] = 5
    else:
    ...
    if i == 0:
        strategy_steps[i]['target_flux'] = 1.0 #Try 1.0 Jy
        strategy_steps[i]['max_directions'] = 1 #Start with DI selfcal
    ...
    strategy_steps[i]['regroup_model'] = False #Don't need to do this
