...
struct Measurement {
  Measurement(size_t rowid, size_t timeIndex, size_t antenna1, size_t antenna2,
  ...
  UVW_Datatype* get_uvw_ptr() { return &uvw.u; }
...
class Grid2D {
 public:
  Grid2D(size_t height = 1, size_t width = 1)
      : m_width(width),
        m_height(height),
  ...
  T& operator()(size_t y, size_t x) { return m_buffer[x + m_width * y]; }

  void init(const T& a) {
    for (unsigned int y = 0; y < get_height(); ++y)
      for (unsigned int x = 0; x < get_width(); ++x) (*this)(y, x) = a;
  }
...
class Grid3D {
 public:
  Grid3D(size_t depth = 1, size_t height = 1, size_t width = 1)
      : m_width(width),
        m_height(height),
        m_depth(depth),
        m_delete_buffer(true),
  ...
  Grid3D(T* data, size_t depth = 1, size_t height = 1, size_t width = 1)
      : m_width(width),
        m_height(height),
        m_depth(depth),
        m_delete_buffer(false),
...

  {
    if (this != &other) {
      ...
      other.m_width = 0;
      other.m_height = 0;
      other.m_depth = 0, other.m_delete_buffer = false;
      other.m_buffer = nullptr;
    }
    return *this;
  }
  ...
  const T& operator()(size_t layer, size_t y, size_t x) const {
    return m_buffer[x + m_width * y + m_width * m_height * layer];
  }

  T& operator()(size_t layer, size_t y, size_t x) {
    return m_buffer[x + m_width * y + m_width * m_height * layer];
  }
  ...
  void init(const T& a) {
    for (unsigned int z = 0; z < get_depth(); ++z)
      for (unsigned int y = 0; y < get_height(); ++y)
        for (unsigned int x = 0; x < get_width(); ++x) (*this)(z, y, x) = a;
  }
...
template <class T>
std::ostream& operator<<(std::ostream& os, const UVW<T>& c) {
  os << "{" << c.u << "," << c.v << "," << c.w << "}";
  return os;
}

template <class T>
std::ostream& operator<<(std::ostream& os, const StationPair<T>& s) {
  os << "{" << s.first << "," << s.second << "}";
  return os;
}
...
template <class T>
std::ostream& operator<<(std::ostream& os, const Matrix2x2<T>& A) {
...
template <class T>
std::ostream& operator<<(std::ostream& os, const Grid2D<T>& grid) {
...
template <class T>
std::ostream& operator<<(std::ostream& os, const Grid3D<T>& grid) {
...
std::ostream& operator<<(
    std::ostream& os, const Measurement<UVW_Datatype, Visibility_Datatype>& c) {
  os << "{" << c.rowid << ", " << c.timeIndex << ", (" << c.antenna1 << ","
     << c.antenna2 << "), " << c.uvw << ", " << c.visibilities << "}";
  return os;
}
