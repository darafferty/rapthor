...
BufferImpl::BufferImpl(BufferSetImpl* bufferset, proxy::Proxy* proxy,
...
double BufferImpl::get_image_size() const { return m_cellWidth * m_gridWidth; }

void BufferImpl::set_cell_size(double height, double width) {
  if (height != width) throw invalid_argument("Only square cells supported.");

  m_cellHeight = float(height);
  m_cellWidth = float(width);
}

double BufferImpl::get_cell_height() const { return m_cellHeight; }

double BufferImpl::get_cell_width() const { return m_cellWidth; }

void BufferImpl::set_w_step(float w_step) { m_wStepInLambda = w_step; }

float BufferImpl::get_w_step() const { return m_wStepInLambda; }
...
const idg::Array1D<float>& BufferImpl::get_shift() const { return m_shift; }

void BufferImpl::set_subgrid_size(const size_t size) { m_subgridsize = size; }

size_t BufferImpl::get_subgrid_size() const { return m_subgridsize; }

void BufferImpl::set_kernel_size(float size) { m_kernel_size = size; }

float BufferImpl::get_kernel_size() const { return m_kernel_size; }

void BufferImpl::set_spheroidal(size_t size, const float* spheroidal) {
  set_spheroidal(size, size, spheroidal);
}

void BufferImpl::set_spheroidal(size_t height, size_t width,
...
void BufferImpl::set_grid(std::shared_ptr<Grid> grid) {
  m_grid = grid;
...
size_t BufferImpl::get_grid_height() const { return m_gridHeight; }

size_t BufferImpl::get_grid_width() const { return m_gridWidth; }
  ...
  for (auto s = 0; s < m_nrStations; ++s) {
    for (auto y = 0; y < m_subgridsize; ++y) {
      for (auto x = 0; x < m_subgridsize; ++x) {
        size_t offset =
            m_subgridsize * m_subgridsize * s + m_subgridsize * y + x;
...
void BufferImpl::set_aterm(size_t timeIndex, const complex<float>* aterms) {
  ...
  // Overwrite last a-term if new timeindex same as one but last element
  // aterm_offsets
  ...
  std::copy(
      aterms, aterms + atermBlockSize * 4,
...
void BufferImpl::copy_grid(size_t nr_polarizations, size_t height, size_t width,
                           complex<double>* grid) {
  //         if (nr_polarizations != m_nrPolarizations)
  //             throw invalid_argument("Number of polarizations does not
  //             match.");
...
void Buffer_set_spheroidal(idg::api::BufferImpl* p, int height, int width,
                           float* spheroidal) {
  p->set_spheroidal(height, width, spheroidal);
}

void Buffer_set_cell_size(idg::api::BufferImpl* p, double height,
                          double width) {
...
void Buffer_copy_grid(idg::api::BufferImpl* p, int nr_polarizations, int height,
                      int width,
                      void* grid)  // pointer complex<double>
{
  p->copy_grid(nr_polarizations, height, width, (complex<double>*)grid);
}
