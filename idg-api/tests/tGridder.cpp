...
BOOST_AUTO_TEST_CASE(gridder) {
  idg::api::options_type options;
  unique_ptr<idg::api::BufferSet> bufferset(
  ...
  bufferset->init_buffers(buffersize, bands, nr_stations, max_baseline, options,
  ...
  // Initialize a-terms
  vector<complex<float>> aterms(
      nr_aterms * nr_stations * subgridsize * subgridsize * 4, 0.);
    ...
    for (auto station = 0; station < nr_stations; ++station) {
      for (auto x = 0; x < subgridsize; ++x) {
        for (auto y = 0; y < subgridsize; ++y) {
          size_t offset =
              4 * subgridsize * subgridsize * nr_stations * aterm_num +
              4 * subgridsize * subgridsize * station + 4 * subgridsize * x +
              4 * y;
