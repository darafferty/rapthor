...
def _has_error_ctest(test_case):
    return test_case.getAttribute('Status') == "error"

def _has_failure_ctest(test_case):
    return test_case.getAttribute('Status') == "failed"
...
def get_tests_metrics(utests_xml_file):
    ...
    if len(test_cases) > 0:
        ...
        test_cases = utests_xml.getElementsByTagName('Test')
        errors = len(list(filter(_has_error_ctest, test_cases)))
        failures = len(list(filter(_has_failure_ctest, test_cases)))
    return {
        'errors': errors,
        'failed': failures,
        'failures': failures,
